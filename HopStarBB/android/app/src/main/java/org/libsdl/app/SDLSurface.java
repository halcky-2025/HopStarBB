...
public class SDLSurface extends SurfaceView implements SurfaceHolder.Callback,
    ...
    public WindowInsets onApplyWindowInsets(View v, WindowInsets insets) {
        if (Build.VERSION.SDK_INT >= 30 /* Android 11 (R) */) {
            // ★ HopStarBB 改修: gesture / mandatorySystemGestures / tappableElement の
            //   inset まで含めると safe area が必要以上に小さくなる
            //   (mandatorySystemGestures が status bar 87px に対して 119px とか上乗せ)。
            //   **実際に視覚的に塞がれる領域** だけに絞る → systemBars (status + nav) +
            //   displayCutout のみ。

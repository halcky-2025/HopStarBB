...
namespace Cyclon
{
    partial class Comment
    {
        ...
        public override Obj exepC(ref int n, Local local, Primary primary)
        {
            ...
            // tag 階層の親 (= 旧 TreeElement* 廃止につき ElementType* に統合)
            var parentte = new Vari("%ElementType*", "%parentte");
            ...
            if (local.comments.Count > 0)
            {
                ...
                else
                {
                    ...
                    if (elemobj.id == "main")
                    {
                        ...
                        // checkTreeElement は %ElementType* を直接返す (= 旧 TreeElementType 中継廃止)
                        var deletes = new Vari("%ListType*", "%deletes");
                        ...
                        ev = new Vari("%ElementType*", "%ev" + LLVM.n++);
                        local.llvm.func.comps.Add(new Call(ev, fv4, thgc, local.llvm.tagparenttes.Count == 0 ? new Vari("%ElementType*", "null") : local.llvm.tagparenttes.Last(), local.llvm.tagparents.Count == 0 ? new Vari("%ElementType*", "null") : local.llvm.tagparents.Last(), deletes, sv, new Vari("i32", "2"), new Vari("i32", elemobj.op.ToString()), fv));
                        local.llvm.tagparents.Add(ev);
                        local.llvm.tagparenttes.Add(ev);   // 統合により tagparents と同じ
                    }
                        ...
                        case ObjType.Div:
                            {
                                ...
                                local.llvm.func.comps.Add(new Call(sv, crs, thgc, strv, new Vari("i32", elemobj.id.Length.ToString()), new Vari("i32", "1")));
                                ev = new Vari("%ElementType*", "%ev" + LLVM.n++);
                                local.llvm.func.comps.Add(new Call(ev, fv4, thgc, local.llvm.tagparenttes.Count == 0 ? new Vari("%ElementType*", "null") : local.llvm.tagparenttes.Last(), local.llvm.tagparents.Count == 0 ? new Vari("%ElementType*", "null") : local.llvm.tagparents.Last(), deletes, sv, new Vari("i32", "21"), new Vari("i32", elemobj.op.ToString()), fv));
                                local.llvm.tagparents.Add(ev);
                                local.llvm.tagparenttes.Add(ev);
                            }
                            break;
                        case ObjType.Sheet:
                            {

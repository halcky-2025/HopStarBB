...
public:
    // ========================================================================
    // Async Job - Posted to file worker thread
    // ========================================================================
    struct FileJob {
        FileDescriptor desc;
        uint64_t offset = 0;
    ...
    FileEngine(std::shared_ptr<ICloudControlApp> cloudApp,
        ...
        m_worker = std::thread(&FileEngine::workerLoop, this);
    }
    ...
    FileEngine(FileEngine&&) = delete;
    FileEngine& operator=(FileEngine&&) = delete;
    ...
    // ========================================================================
    // Async Read - Posts job to file worker thread
    // ========================================================================
    // The callback is invoked ON THE WORKER THREAD. The caller is responsible
    // for marshaling the result back to whatever thread it cares about
    // (e.g. via SDL_PushEvent, coroutine resume, etc.)
    void readAsync(const FileDescriptor& desc,
                   std::function<void(ReadResult)> callback,
                   uint64_t offset = 0,
                   uint64_t length = UINT64_MAX) {
        if (!callback) return;
...
private:
    ...
    // ========================================================================
    // File worker thread loop - processes queued FileJobs
    // ========================================================================
    void workerLoop() {
        while (true) {
            FileJob job;
            {
                ...
                m_jobCv.wait(lk, [&] { return m_workerStop.load() || !m_jobs.empty(); });
                if (m_workerStop.load() && m_jobs.empty()) return;
                job = std::move(m_jobs.front());
                m_jobs.pop_front();
            }
            // Execute the synchronous read on this worker thread
            ReadResult result = read(job.desc, job.offset, job.length);
            if (job.callback) {
                try { job.callback(std::move(result)); }
                catch (...) { /* swallow - worker must not die */ }
            }
        }
    }
    ...
    // File worker thread state
    std::thread m_worker;
    std::deque<FileJob> m_jobs;

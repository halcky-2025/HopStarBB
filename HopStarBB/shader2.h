...
struct RenderResources {
    ...
    bgfx::UniformHandle tex2Uniform = BGFX_INVALID_HANDLE;
    bgfx::UniformHandle tex3Uniform = BGFX_INVALID_HANDLE;
...
struct DrawCommand {
    ...
    PointI* fbsize;
    // ── タイル描画情報 ──
    // tileIndex >= 0 の時、このコマンドはタイル展開済み。submit 時に
    //   master.getTiledTexture(tiledImageId)->tiles[tileIndex] をルックアップして
    //   その時点の fbo / size を使う (古いポインタの dangling を防ぐ)。
    // tileIndex == -1 (デフォルト) なら従来通り targetFBO / fbsize を使う。
    ...
    bgfx::TextureHandle* texture3 = nullptr;
    bgfx::TextureHandle* texture4 = nullptr;
    ...
    //   9: 右辺タブ突起  10: 左辺タブ突起
    //  11: 下辺タブ突起  12: 上辺タブ突起
    //  小数部: 辺上の位置 (0.0=始点, 0.5=中央, 1.0=終点)
    float cornerPattern = 0.0f;
    // シザー (clip rect)。NULL なら scissor 無し。LayerInfo::push が currentScissor を自動コピー。
    //   描画先 fbo (winFb / offscreen FBO) のピクセル空間 (左上原点)。
    //   同一 RenderPass 内に異なる scissor の cmd が混ざらないよう、grouping key に含める。
...
struct UnifiedDrawCommand : DrawCommand {
    ...
    float shadowX = 0.0f;
    float shadowY = 0.0f;
    float shadowBlur = 0.0f;  // packInstanceでaaと共にhalf16パック → data3[3]
...
inline void packInstance(UnifiedDrawCommand& cmd, UnifiedInstanceData& out, DrawCommandType type) {
    ...
    if (type == DrawCommandType::Image || type == DrawCommandType::RawImage) {
        ...
        out.data1[1] = cmd.backUVMinX;  // Fill/Pattern: ジグソータブ半径
        out.data1[2] = cmd.angle;      // uvMin.x
        out.data1[3] = cmd.curlRadius; // uvMin.y
    }
    ...
    uint8_t viewId)
{
    ...
    uint32_t total = static_cast<uint32_t>(sorted.size());
    DrawCommandType batchType = sorted[0]->type;

    // テクスチャ設定（ポインタをデリファレンス）── batch 全体で同じ前提
    ...
    bgfx::TextureHandle tex3 = resources.placeholderTexture;
    if (sorted[0]->texture3 && bgfx::isValid(*sorted[0]->texture3)) tex3 = *sorted[0]->texture3;
    bgfx::TextureHandle tex4 = resources.placeholderTexture;
    if (sorted[0]->texture4 && bgfx::isValid(*sorted[0]->texture4)) tex4 = *sorted[0]->texture4;
    ...
    uint64_t stateFlags = (blendMode > 0.5f)
    ...
    // bgfx の per-frame instance data buffer は容量上限あり (= デフォルト ~5MB / 80B = ~64K)。
    // 大きい batch (= 1000+ cmd) は分割サブミットしないと allocInstanceDataBuffer が assert する。
    // getAvailInstanceDataBuffer で残量を確認しつつチャンクで回す。
    uint32_t pos = 0;
    while (pos < total) {
        uint32_t remain = total - pos;
        uint32_t avail = bgfx::getAvailInstanceDataBuffer(remain, 80);
        if (avail == 0) break;   // 容量無し (= 諦める、フレーム空け待ち)
        bgfx::InstanceDataBuffer idb;
        bgfx::allocInstanceDataBuffer(&idb, avail, 80);
        if (!idb.data) break;
        for (uint32_t i = 0; i < avail; ++i) {
            UnifiedInstanceData inst;
            packInstance(*sorted[pos + i], inst, batchType);
        ...
        bgfx::setTexture(2, resources.tex2Uniform, tex3);
        bgfx::setTexture(3, resources.tex3Uniform, tex4);
    ...
    uint8_t viewId)
{
    ...
    auto flushBatch = [&](size_t start, size_t end) {
        uint32_t total = static_cast<uint32_t>(end - start);
        if (total == 0) return;

        const uint32_t stride = sizeof(PageCurlInstanceData);
        // 大量 batch 時は per-frame instance buffer の容量上限で
        // allocInstanceDataBuffer が assert する。
        // getAvailInstanceDataBuffer で残量チェックしながらチャンク sub-submit。
        uint32_t pos = 0;
        while (pos < total) {
            uint32_t remain = total - pos;
            uint32_t avail = bgfx::getAvailInstanceDataBuffer(remain, stride);
            if (avail == 0) break;
            bgfx::InstanceDataBuffer idb;
            ...
            for (uint32_t i = 0; i < avail; ++i) {
                ...
                packPageCurlInstance(*sorted[start + pos + i], inst);
                std::memcpy(idb.data + i * stride, &inst, stride);
            }
            bgfx::setVertexBuffer(0, resources.quadVB);
            bgfx::setIndexBuffer(resources.quadIB);

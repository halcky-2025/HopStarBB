...
public:
    ...
    const GlyphInfo& addGlyph(FontId font, uint32_t cp, SDL_Color color,
        ...
        gi.bearingX = minx;
        gi.bearingY = 0;
        // overhang 防止: quad 右端 (= bearingX + width) が advance を超えるなら advance を伸ばす。
        // overhang のある字形 (italic, ぶら下がり) で次文字に被って描画されるのを防ぐ。
        {
            int rightEdge = gi.bearingX + (int)gi.width;
            if (rightEdge > gi.advance) gi.advance = rightEdge;
        }
        gi.lastUsedFrame = currentFrame_;
